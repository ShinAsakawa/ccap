...
class TLPA():
    '''TLPA (藤田 他, 2000, 「失語症語彙検査」の開発，音声言語医学 42, 179-202) の語彙を検証データセットと考える。
    ...
    * `training_data`, `tlpa_data` という辞書が本体である。
    各辞書の項目には，さらに
    `tlpa.tlpa_data[0].keys() = dict_keys(['orig', 'ortho', 'phone', 'ortho_ids', 'phone_ids', 'semem'])`
    ...
    * <PAD>: 埋め草トークン
    ...
    このクラスで定義されるデータは 2 つの辞書である。すなわち 1. training_data, 2. tlpa_data である。
    各辞書は，次のような辞書項目を持つ。
    ```
    {0: {'orig': 'バス',
    'yomi': 'ばす',
    ...
    def __init__(self, 
        ...
        self.mora_vocab=['<EOW>', '<SOW>', '<UNK>', '<PAD>', \
        ...
        self.mora_p_vocab = ['<EOW>', '<SOW>', '<UNK>', '<PAD>', \
        ...
        self.mora_freq = {'<EOW>':0, '<SOW>':0, '<UNK>':0, '<PAD>':0}
        self.mora_p = {}

        # 最初に TLPA データを読み込む
        self.tlpa_vocab = self.get_tlpa_vocab()  # tlpa データを読み込む

        # NTT 日本語語彙特性データから，`self.training_data` を作成
        ...
        for orth in tqdm(self.training_vocab):
            ...
            self.training_data[i] = {'orig': orth, 'yomi': _yomi,
        ...
        for orth in self.tlpa_vocab:
            i = len(self.tlpa_data)

            _yomi, _phon, _phon_r, _orth, _orth_r, _mora, _mora_r = self.get7lists_from_orth(orth)
            phon_ids, phon_ids_r, orth_ids, orth_ids_r, mora_ids, mora_ids_r = self.get6ids(_phon, _orth, _yomi)
            _yomi, _mora1, _mora1_r, _mora, _mora_ids, _mora_p, _mora_p_r, _mora_p_ids, _mora_p_ids_r, _juls = self.yomi2mora_transform(_yomi)
            self.tlpa_data[i] = {'orig': orth, 'yomi': _yomi,
                                 'ortho':_orth, 'ortho_ids': orth_ids, 'ortho_r': _orth_r, 'ortho_ids_r': orth_ids_r,
                                 'phone':_phon, 'phone_ids': phon_ids, 'phone_r': _phon_r, 'phone_ids_r': phon_ids_r,
                                 'mora': _mora1, 'mora_r': _mora1_r, 'mora_ids': _mora_ids, 'mora_p': _mora_p,
                                 'mora_p_r': _mora_p_r, 'mora_p_ids': _mora_p_ids, 'mora_p_ids_r': _mora_p_ids_r,
